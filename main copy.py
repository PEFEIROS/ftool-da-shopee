...
def showInfo(lines, forcas, botaoBarra, vinculos, momentos, root, canvas):
    ...
    # info.geometry('480x360')
    # info.minsize(300, 300)
    # info.maxsize(300, 300)
    indiceBarras = []
    infoForcas = [{}]
    infoVinculos = [[]]
    infoMomentos = [[]]
        ...
        try:
            if (canvas.coords(forcas[i]["f1"])[0] != 0 or
                ...
                canvas.coords(forcas[i]["f3"])[0] != 0):
                    infoForcas.append({"cx1":canvas.coords(forcas[i]["f1"])[0]})
                    infoForcas.append({"cy1":canvas.coords(forcas[i]["f1"])[3]})
                    infoForcas.append({"cx2":canvas.coords(forcas[i]["f2"])[0]})
                    infoForcas.append({"cy2":canvas.coords(forcas[i]["f2"])[3]})
                    infoForcas.append({"cx3":canvas.coords(forcas[i]["f3"])[0]})
                    infoForcas.append({"cy3":canvas.coords(forcas[i]["f3"])[3]})
                    
                    modulo = anguloForca(canvas, forcas[i]["f1"])[0]
                    angulo = anguloForca(canvas, forcas[i]["f1"])[1]
                    infoForcas.append({"fx1":round(-math.cos(np.deg2rad(angulo))*modulo)})
                    infoForcas.append({"fy1":round(-math.sin(np.deg2rad(angulo))*modulo)})
                    
                    modulo = anguloForca(canvas, forcas[i]["f2"])[0]
                    angulo = anguloForca(canvas, forcas[i]["f2"])[1]
                    infoForcas.append({"fx2":round(-math.cos(np.deg2rad(angulo))*modulo)})
                    infoForcas.append({"fy2":round(-math.sin(np.deg2rad(angulo))*modulo)})
                    
                    modulo = anguloForca(canvas, forcas[i]["f3"])[0]
                    angulo = anguloForca(canvas, forcas[i]["f3"])[1]
                    infoForcas.append({"fx3":round(-math.cos(np.deg2rad(angulo))*modulo)})
                    infoForcas.append({"fy3":round(-math.sin(np.deg2rad(angulo))*modulo)})
        except:
            pass
    ...
    # text = Text(info, width=40, height=40, background='#abaaa9')
    # text.insert('0.0', 'Indices das barras\n')
...
def addMomento(num, theta, canvas, lines, momentos, janela):
    ...
    sliderLabel.grid(row=0, column=0, padx=2, sticky=tk.W)
    ...
    slider.grid(row=0, column=0, ipadx=50)
...
def addVinculo(num, theta, canvas, lines, vinculos, janela):
    ...
    options = ["Apoio Simples", "Apoio Duplo", "Engaste", 'Junção']
...
def addForca(num, theta, canvas, lines, forcas, janela):
    ...
    barraLabel = []
    barraLabel.insert(forcas, canvas.create_text(x+30, y-100+20, text="Forca {}".format(forcas), 
    ...
    aplicar = tk.Button(janela, text="Aplicar Força", 
                        command=lambda: aplicarForca(num, theta, forcaVar, sliderVar, 
                                                     canvas, forcas, lines, barraLabel))
    aplicar.grid(row=4, column=0, padx=5, columnspan=2, pady=5, sticky=tk.W)
    ...
    aplicar = tk.Button(janela, text="Aplicar Angulo", command=lambda: mudarAnguloForcas(forcas, canvas, anguloVar, barraLabel))
    aplicar.grid(row=2, column=1, padx=5, columnspan=2, pady=5, sticky=tk.W)
    
    
def mudarAnguloForcas(forcas, canvas, anguloVar, barraLabel):
    ...
    canvas.coords(barraLabel, x+30, y+20)
    
    
def aplicarForca(num, theta, forca, sliderVar, canvas, forcas, lines, barraLabel):
    slider = sliderVar.get()
    try:
    ...
    canvas.coords(forcas, newX, newY-100, newX, newY)
    canvas.coords(barraLabel, newX+30, newY-100+20)
